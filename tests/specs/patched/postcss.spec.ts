...
export default testSuite(({ describe }) => {
	describe('PostCSS', ({ test, describe }) => {
		...
		describe('.d.ts', ({ test }) => {
			...
			test('bundled types - single file', async () => {
				await using fixture = await createFixture({
					'index.js': 'export * as button from \'./components/Button.module.css\'; export * as card from \'./components/Card.module.css\';',
					...
					`,
					'components/Card.module.css': outdent`
						.card {
							padding: 1rem;
						}
						.title {
					...
					'test.ts': outdent`
						...
						const _button: string = button.button;
						const _icon: string = button.icon;
						const _card: string = card.card;
						const _title: string = card.title;
					`,
					...
					// Symlink node_modules for TypeScript and deps
					node_modules: ({ symlink }) => symlink(path.resolve('node_modules')),
				});
				...
				await viteBuild(fixture.path, {
					plugins: [
						patchCssModules({
							generateSourceTypes: 'types.d.ts',
						}),
					],
				});

				const files = await fixture.readdir('.', { recursive: true });
				expect(files).toContain('types.d.ts');

				const dts = await fixture.readFile('types.d.ts', 'utf8');

				// Debug: log the generated types
				console.log('Generated types.d.ts:');
				console.log(dts);

				// Check file header
				expect(dts).toContain('Generated by vite-css-modules');

				// Check module declarations with root-relative paths
				expect(dts).toContain('declare module \'./components/Button.module.css\'');
				expect(dts).toContain('declare module \'./components/Card.module.css\'');

				// Check Button.module.css exports
				expect(dts).toMatch(/button.*string/);
				expect(dts).toMatch(/icon.*string/);

				// Check Card.module.css exports
				expect(dts).toMatch(/card.*string/);
				expect(dts).toMatch(/title.*string/);

				// Run TypeScript to verify types are resolvable
