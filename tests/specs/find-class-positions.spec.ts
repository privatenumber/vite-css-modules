...
import { findClassPositions } from '../../src/plugin/generate-dts-sourcemap.ts';
...
.header { color: blue; }`;
		...
		expect(result.get('button')).toStrictEqual({ line: 1, column: 1 });
		expect(result.get('header')).toStrictEqual({ line: 2, column: 1 });
	});
	...
	test('compound selectors', () => {
		...
		expect(result.get('button')).toStrictEqual({ line: 1, column: 1 });
		expect(result.get('active')).toStrictEqual({ line: 1, column: 8 });
	});

	test('multi-selector rule', () => {
		const css = `.foo, .bar { color: red; }`;
		const result = findClassPositions(css, ['foo', 'bar']);
		expect(result.get('foo')).toStrictEqual({ line: 1, column: 1 });
		expect(result.get('bar')).toStrictEqual({ line: 1, column: 7 });
	});
...
.button { color: red; }`;
		const result = findClassPositions(css, ['button']);
		expect(result.get('button')).toStrictEqual({ line: 2, column: 1 });
	});
...
.button { color: red; }`;
		...
		expect(result.get('label')).toStrictEqual({ line: 1, column: 1 });
		expect(result.get('button')).toStrictEqual({ line: 4, column: 1 });
	});
...
.button { color: red; }`;
	...
	&.active {
		...
		const result = findClassPositions(css, ['button', 'active']);
		expect(result.get('button')).toStrictEqual({ line: 1, column: 1 });
		expect(result.get('active')).toStrictEqual({ line: 3, column: 3 });
	});
...
.sibling
  color: green`;
		...
		expect(result.get('button')).toStrictEqual({ line: 1, column: 1 });
		expect(result.get('active')).toStrictEqual({ line: 4, column: 4 });
		expect(result.get('sibling')).toStrictEqual({ line: 7, column: 1 });
	});
...
.button { color: red; }`;
		...
		expect(result.get('icon')).toStrictEqual({ line: 1, column: 1 });
		expect(result.get('button')).toStrictEqual({ line: 4, column: 1 });
	});
...
.header { color: red; }`;
		...
		expect(result.get('grid')).toStrictEqual({ line: 1, column: 1 });
		expect(result.get('header')).toStrictEqual({ line: 4, column: 1 });
	});
...
.mixin { display: none; }`;
		...
		expect(result.get('button')).toStrictEqual({ line: 1, column: 1 });
		expect(result.get('mixin')).toStrictEqual({ line: 5, column: 1 });
	});
...
.normal { color: blue; }`;
		...
		expect(result.get('foo:bar')).toStrictEqual({ line: 1, column: 1 });
		expect(result.get('normal')).toStrictEqual({ line: 2, column: 1 });
	});
...
.button {
	color: red;
		...
		expect(result.get('container')).toStrictEqual({ line: 4, column: 1 });
		expect(result.get('expanded')).toStrictEqual({ line: 7, column: 3 });
		expect(result.get('button')).toStrictEqual({ line: 12, column: 1 });
	});
...
.header { color: blue; }`;
		const result = findClassPositions(css, ['button', 'header']);
		expect(result.get('button')).toStrictEqual({ line: 5, column: 1 });
		expect(result.get('header')).toStrictEqual({ line: 6, column: 1 });
	});
});
