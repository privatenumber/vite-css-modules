...
var cssClassPositions = require('css-class-positions');
...
const slash = (filePath) => filePath.replaceAll("\\", "/");
...
const dtsHeaderLineCount = dtsHeaderLines.length + 1;
const dtsTemplate = (code, sourceMappingURL) => {
  let result = `${dtsHeaderLines.join("\n")}${code ? `
...
const cssModules = (config, patchConfig, originalCssCache) => {
  ...
  const loadTransformer = isLightningCss ? Promise.resolve().then(function () { return require('./lightningcss-KtT4ee4x.cjs'); }) : Promise.resolve().then(function () { return require('./index-DKmu58fD.cjs'); });
  let transform;
  const exportMode = patchConfig?.exportMode ?? "both";
  const declarationMap = patchConfig?.declarationMap ?? getTsconfig.getTsconfig(config.root)?.config.compilerOptions?.declarationMap ?? false;
  const activeEdges = /* @__PURE__ */ new Map();
  const findCyclePath = (fromId, targetId, visited = /* @__PURE__ */ new Set()) => {
    if (fromId === targetId) {
      return [fromId];
    }
          ...
          if (filePath && cssModuleRE.test(filePath)) {
            ...
            if (fileExists) {
              ...
              const sourceMapOptions = declarationMap && originalCss ? {
                ...
                classPositions: cssClassPositions.cssClassPositions(originalCss, { fileName: filePath })
              } : void 0;
...
const patchCssModules = (patchConfig) => {
  const originalCssCache = /* @__PURE__ */ new Map();
  const plugin = {
