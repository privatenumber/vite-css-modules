...
const importStatement = (specifiers, source) => `import${specifiers ? `{${specifiers}}from` : ""}${JSON.stringify(source)};`;
const importsToCode = (imports) => Array.from(imports).map(
...
const exportsToCode = (exports) => {
  ...
  const namedExports = `export {${exportedVariables.map(
    ([jsVariable, exportName]) => jsVariable === exportName ? jsVariable : `${jsVariable} as ${exportName}`
  ).join(",")}};`;
...
const generateEsm = (imports, exports) => importsToCode(imports) + exportsToCode(exports);

const cssModuleRE = /\.module\.(css|less|sass|scss|styl|stylus|pcss|postcss|sss)(?:$|\?)/;
const pluginName = "vite:css-modules";
const postfixRE = /[?#].*$/;
const cleanUrl = (url) => url.replace(postfixRE, "");
const cssModules = (config) => {
  ...
  const cssModuleConfig = {
    ...isLightningCss ? lightningCssOptions.cssModules : cssConfig.modules
  };
  const loadTransformer = isLightningCss ? Promise.resolve().then(function () { return require('./lightningcss-cbed3ada.cjs'); }) : Promise.resolve().then(function () { return require('./index-14700a08.cjs'); });
  const loadExports = async (context, requestId, fromId) => {
    ...
    return loaded.meta[pluginName].exports;
  };
  return {
    name: pluginName,
    buildStart: async () => {
    ...
    async transform(inputCss, id) {
      ...
      let counter = 0;
      const preserveOriginalExport = !("localsConvention" in cssModuleConfig && (typeof cssModuleConfig.localsConvention === "function" || cssModuleConfig.localsConvention === "camelCaseOnly" || cssModuleConfig.localsConvention === "dashesOnly"));
      ...
      await Promise.all(
        Object.entries(cssModule.exports).map(async ([exportName, exported]) => {
          if (typeof exported === "string") {
            exports[exportName] = {
              value: exported,
              exportAs: /* @__PURE__ */ new Set([exportName])
            };
          } else {
  ...
  configResolved: (config) => {
    ...
    viteCssPostPlugin.transform = async function(jsCode, id, options) {
      if (cssModuleRE.test(id)) {
        ...
        const css = info.meta[pluginInstance.name].css;
