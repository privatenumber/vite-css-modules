...
const createTypeFileConfigFingerprint = async (mode, configPath, configDependencies = [], fallbackConfig) => {
...
const createTypeFileFingerprint = (sourceCode, configFingerprint, declarationMap) => createHash("sha256").update("vite-css-modules-cli-cache-v1").update("\0").update(configFingerprint).update("\0").update(declarationMap ? "1" : "0").update("\0").update(sourceCode).digest("hex").slice(0, 16);
