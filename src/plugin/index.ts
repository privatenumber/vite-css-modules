...
const generateBundledTypes = (
	modules: Map<string, CssModuleInfo>,
	rootDirectory: string,
) => {
	const declarations = Array.from(modules.entries()).map(([filePath, info]) => {
		// Generate relative path from root for module specifier
		const moduleSpecifier = `./${path.relative(rootDirectory, filePath).replace(/\\/g, '/')}`;

		// Generate types for this module
		const moduleTypes = generateTypes(
			info.exports,
			info.exportMode,
			info.allowArbitraryNamedExports,
		);

		// Extract just the variable and export declarations
		// Skip the header comments and empty lines
		const lines = moduleTypes.split('\n');
		...
		return `declare module '${moduleSpecifier}' {\n${content}\n}`;
	});

	// Generate file header + all module declarations
...
export type PatchConfig = {
	 ...
	 *
	 * - `true`: Creates a `.d.ts` file next to each CSS module (e.g., `style.module.css.d.ts`)
	 * - `string`: Path to a single bundled declaration file containing all CSS module types
	 *   (e.g., `'src/css-modules.d.ts'`)
	 *
	 * @example
	 * // Adjacent .d.ts files
	 ...
	 * @example
	 * // Single bundled file
	 * generateSourceTypes: 'src/types/css-modules.d.ts'
	 */
	generateSourceTypes?: boolean | string;
};
...
export const cssModules = (
	...
	// Registry for bundled type generation
	const cssModulesRegistry = new Map<string, CssModuleInfo>();
	const bundledTypesPath = typeof patchConfig?.generateSourceTypes === 'string'
		? path.resolve(config.root, patchConfig.generateSourceTypes)
		: null;
	...
	return {
			...
			async handler(inputCss, id) {
				...
				if (patchConfig?.generateSourceTypes) {
					...
					if (filePath && cssModuleRE.test(filePath)) {
						...
						if (fileExists) {
							if (bundledTypesPath) {
								// Collect module info for bundled types
								cssModulesRegistry.set(filePath, {
									exports,
									exportMode,
									allowArbitraryNamedExports,
								});

								// In watch mode (dev), write bundled file immediately
								if (this.meta.watchMode) {
									const bundledTypes = generateBundledTypes(cssModulesRegistry, config.root);
									await writeFile(bundledTypesPath, bundledTypes);
								}
							} else {
								// Old behavior: write adjacent .d.ts file
								await writeFile(
								`${filePath}.d.ts`,
		...
		async generateBundle() {
			// Write bundled types during build (not in watch mode)
			if (bundledTypesPath && !this.meta.watchMode && cssModulesRegistry.size > 0) {
				const bundledTypes = generateBundledTypes(cssModulesRegistry, config.root);
				await writeFile(bundledTypesPath, bundledTypes);
			}
		},
